...
#+end_src
** ポインタ型
- ポインタ型を生成するときは ~&~ を使う。これは変換関数のようなもの
- ポインタ型を示すときは ~*Person~ となる
...
* Go言語へのコントリビュート準備
** TODO [[https://go.dev/doc/contribute][Contribution Guide - The Go Programming Language]]
:LOGBOOK:
...
:END:
どうやって言語をコンパイルするのだろうか。

src/all.bash にシェルスクリプトがあり、それでコンパイルできるかをチェックできる。
...
テストを実行するとinternal packagesがimportできないため失敗する。やり方があるはず。

結論: 単純にコマンドの引数が間違っている。 *拡張子や_testは不要* 。また、引数はファイルシステムではなく、モジュールの構造である。だから、 ~./fmt~ とかも不可。エラー内容が不親切でわかりにくい。

#+caption: 例えばこんな感じのエラー
#+begin_quote
$ go test fmt/errors_test.go
fmt_test.go:10:2: use of internal package internal/race not allowed
#+end_quote
...
$ go test -v errors
=== RUN   TestNewEqual
--- PASS: TestNewEqual (0.00s)
=== RUN   TestErrorMethod
#+end_src

#+caption: 単体ファイル指定では実行できないものもあった。全体ではテストを実行できる
#+begin_src shell
$ go test -v fmt/...
  === RUN   TestErrorf
...
- ~sequencer~ とは
  - スライス等ではないが、prevフィールドで辿れるグループ構造に見える
  - ファイル処理を含む無名関数が入ってるぽい
  - エラーを出す無名関数を入れることもある
- go/parser packageのparse()が、パッケージ指定なしで使えるのはなぜか(LSPが定義を追えない)

* Tasks
** TODO 何も出ずに失敗するコマンド

↓コマンド自体はナンセンスだが、何も出ないのはおかしそう。とはいえ性善説な設計なので、これでよいのか。バカげたものをいちいちカバーしてたら見通しが悪くなるからな。

#+caption: 既にインストールされてるからか。1文字変えると別のエラーが出るので、一致していることが何か要因になっていそう
#+begin_src shell
go install internal/diff
#+end_src
...
Goでインタプリタを書く本の続編。
** TODO [[https://andmorefine.gitbook.io/learn-go-with-tests/][テスト駆動開発でGO言語を学びましょう - テスト駆動開発でGO言語を学びましょう]]
Go入門。
...
:END:
わかりやすいという入門書。
** WIP [[https://tatsu-zine.com/books/go-web-programming][Goプログラミング実践入門 標準ライブラリでゼロからWebアプリを作る【委託】 - 達人出版会]]
:LOGBOOK:
