...
- gitで1つのファイルに対して時系列を比較することはできないか、それができればいちいち保存するのは必要なくなり、毎回新規作成でよくなりシンプルだ。汎用性も高いように思える。データの修正がしづらいか。不正な形式や値が交じるとまずい

** TODO ローカルパッケージを読み込む方法
