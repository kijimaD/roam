...
関数に進んだ． 1番目の例では既に定義されていたmapcanを使い， 2番目の例
では新しいユーティリティfind2を書いたが， 全体的な原則は同じだ： 一般部
分と個別部分を混ぜ合わせるのでなく， 一般部分を定義して個別部分を引数と
して渡すこと．
#+end_quote

filterは関数と1個のリストを取り， その関数がリストに適用されたときに非
...
#+end_src
** マッピング
(ace-link-setup-default)でマッピングしているところを見つけた。参考になるかもしれない。
** ディレクトリ探索
...
で関数を確認。adviceされてるとわかったので、
(advice-remove '登録関数 'debug--implement-debug-on-entry)
で削除して元に戻った。
** すべてdebug対象に
(edebug-all-defs)
...
オフにするときは(edebug-all-defs)をトグルしてオフにしてから、region指定してM-x eval-region
** 外部コマンド系パッケージ
