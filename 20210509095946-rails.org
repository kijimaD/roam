...
データを整合するときに使う。
** バリデーションと一致してないレコード探知
モデルバリデーションでpresence: trueなのに、レコードではnilが残っていると、別のカラムをupdateするときにバリデーションが弾かれて不便になる。
こういうのはレコードからnilをちゃんと消してモデルとDBのバリデーションを一致させるとよい。

まずnilをなくす。それからテーブルのバリデーションを追加する。

モデルバリデーションは入出力のみ監視する。だから既存レコードに残っている可能性がある。
DBバリデーションは、既存レコードにも入ってないことを保証できる。
...
この場合、編集できなくて不便。検知してテーブルにも制約をかけると安全になる。
