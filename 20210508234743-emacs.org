...
** バッファ管理
rails開発で処理が多くのファイルに分散していて、同じような名前のファイルが多い場合、集中のスイッチングコストが大きくなることがわかった。今まではあまりファイル分散してなかったので ~counsel-switch-buffer~ ひとつで問題なかったが、タスクやコードの構造によって問題があることがわかった。ファイル移動しようとするたびに思考が中断される。
...
** TODO 定義方法の違いはなにか                                      :Write:
同じC言語部分のソースコードでも、DEFUNを使ってlispに書かれたコードと、生のCっぽく書かれたものがある。違いはなにか。
