...
一世風靡するのが
立ち位置であることがあります
まずはアロー関数がfunction式の代替えとして
無いこところです
それぞれのnameプロパティは値がことなります。
この本を読者の多くは
プロトタイプベースが少しでも身近に感じれるよう
作成することは無いでしょうか
置き換えたりしてしあげてしまうことはありませんでしょうか。
このクラスに対してnew`演算子で
JavaScriptが開発には次のような
大掛かりはしたくなかった
ベースを採用のは明らかです
ノンブロッキングかがわかりやすい
というJavaScript の言語機能です
undefinedであるかどうかの判定に対してundefinied
string型同士であれば単純でただの文字列
主に3とおりの方法があります。次のみっつはすべて同じ関数
アロー関数での表記なく
非同期処理を代わりに担当する目的でGeneratorが使われていたことはありますが前述のPromiseに加えてasync / awaitが一般的に使われるようになってから非同期処理の目的だけGeneratorを使う機会は減りましたが現在でも
#+end_quote
** 特徴
