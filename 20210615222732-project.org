...
** claudeにクエリするツール
:LOGBOOK:
CLOCK: [2025-09-13 Sat 12:47]--[2025-09-13 Sat 13:12] =>  0:25
CLOCK: [2025-09-13 Sat 12:21]--[2025-09-13 Sat 12:46] =>  0:25
CLOCK: [2025-09-13 Sat 10:17]--[2025-09-13 Sat 10:42] =>  0:25
:END:

- 複数プロジェクトに対応する
- 表示はGitHub ActionsのCI画面のイメージ
- ステータス表示できる。入力待ち / AI待ち
- ブラウザから別端末からもできれば便利そう
- もちろんそのままコンソールに入力できる
- ブラウザ型の端末があれば、それでいい、となるな
- LANでしか使わないのでセキュリティは考えなくていいが、ディレクトリから出られないようにしておくと安心な感じがする

メモ。

- talkを作成したときにClaudeへリクエストしてsession idを発行してセットする
- クエリを投げるときはsession idを必須にする
- OpenAPIファイルの各フィールドをすべて参照化して共通化する。適用フォーマットを共通化/強化する。同じフィールドなのに仕様が違うのを防ぐのと、型を生成するため

** 理解を確かめるクイズ
本当に理解できているか、生成AIにクイズを出させると確かめられることに気づいた。

大量にテキストで登録しておいて、ランダム表示する。

** 定型文登録
mozcに定型文を登録して、コード管理する。
