...
** 定点録画サーバ
:LOGBOOK:
CLOCK: [2025-08-28 Thu 20:04]--[2025-08-28 Thu 20:29] =>  0:25
:END:

- 複数の定点カメラで、各デバイスごとにタイムラプス画像を保存する
- サムネイル+タイムスタンプ管理で一覧表示できる。
- ブラウザで任意時点で任意のカメラ画像にジャンプ、ズーム・パン操作可能。コンビニにある監視カメラの表示端末のような感じ
- 一定時間ごとにタイムラプス画像を結合して動画を生成する
