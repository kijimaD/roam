...
Goを中心としたバックエンド開発を得意とするソフトウェアエンジニアである。4年以上の実務経験を通じて、Webアプリケーションの設計、開発、テスト、運用まで一貫して担当してきた。CI/CD環境の構築による開発プロセスの改善や、レガシーシステムの移行・リファクタリングといった課題解決に強みがある。
...
- アピールポイント
  ...
  こまめな設計の共有や相談、焦点の明確なPRの提出によって、作業の手戻りを最小限に抑えた。
...
- 普段から使い慣れているLinuxベースのマシンで開発できること(ホストマシンがLinuxベースであること)
- 集中しやすい静かな環境での作業と、チームとの対面コミュニケーションのバランスを取るため、週1~2回程度の出社頻度であること
