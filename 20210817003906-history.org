...
| フェーズ     | 仕様変更および追加開発                                    |
| チーム規模   | 3人程度(全員がバックエンド、フロントエンド、インフラそれぞれ担当) |
| 担当範囲(横) | 主にバックエンド開発                                     |
| 担当範囲(縦) | 実装設計、コーディング、テスト、レビュー                     |
| 使用技術     | Go, React, AWS(特にLambda), GitHub Actions            |
| 期間        | 2024-09 ~ 現在                                        |
...
- 担当詳細
  ...
  リリース後のフェーズから参画し、主に仕様変更…要件の変化による、大規模な権限構造の改修…を担当した。今後のビジネスや販売プランに合わせて変更の可能性が高い部分を見極め、柔軟に変更できるようにした。
  仕様変更前に、テスト全体を改修した。グローバルなfixtureレコードに依存し、各ケース個別の変更が難しくなっていたテストを、各テストケースでレコードを用意するように変更し仕様変更に対応しやすくした。

- アピールポイント

  粒度の異なるテストで守りつつ、仕様変更にかかる破壊的変更がなるべく少なくなるように設計/実装した。稼働中のシステムへの影響を最小限にし、安全に作業を進めた。実装は期限内に完了し、仕様内容にかかる不具合報告は0件だった。
...
- 自治体向け製品の制約(LGWAN)で、本番環境はインターネットに出られないため、SaaSは利用できない点
