...
| 状況        | 開発 → 自治体への納入 → 運用および追加開発                                          |
| チーム規模   | バックエンド1人(自分)、他社混合のフロントエンド開発者5~10人、開発元会社のPM1人            |
...
- ↓Web版の続き
- バックエンドサーバを全く同じAPI仕様のまま[[id:7cacbaa3-3995-41cf-8b72-58d6e07468b1][Go]]で書き直した
  - Web版とデスクトップ版を共用するため
...
- 自治体向け製品の制約(LGWAN)で、本番環境はインターネットに出られない
  - したがってデプロイ時は変更したソースコードさえも、手元のマシンから直接送信しなければならない状況(開発元会社のバージョン管理システムは社内ネットワークとしかつながっておらず、直接転送できない)
  - インターネットがつながる社内マシンでDockerイメージをビルドし、tarで送る + Dockerイメージをtarからロードするスクリプトを書いた
  - パッケージインストールすらできないので、依存関係を固める + ローカルインストールするスクリプトを書いた
- 開発元会社の制約で、開発時も基本的にクラウドサービス等が利用できない
  - AWS, Docker Hub, GitHub等は /チームとして/ 利用できない
- 依頼元の強い要望で、新規開発ではあるが非常に古いバージョンのプログラミング言語、Webフレームワークを使用した。情報が出てこなかったりライブラリがないので困った
- 自社の人員は1人だったのでコードレビュー相手がいなかった。心情としてさみしかった
...
- OpenAPIでスキーマ駆動開発をリードした。フロントエンド用SDKの提供・サーバコードを生成して効率的に開発した
- ネットワークの強い制約のなかで、シェルスクリプトでデプロイを自動化した。結果、本番環境での作業ミスが発生しなかった
- 開発元の製品開発の責任者と文書による合意を常にとり、トラブルや認識ミスが発生しなかった
...
数年アプリケーションプログラマーをやって、楽しく、やりがいも感じている。ずっと仕事にしていきたいと考えているが、まだその中でどうなりたいかは具体化していない。これを一生のしごとにしたいという分野にはまだ出会っていない。具体的でない部分もあるが、今のところどうなりたいかを示す。

職種。

- ~MUST~ プログラマー
  - 実際に手を動かして開発するのが楽しいから。やりがいを感じるから
  - プライベートでの趣味を相互に活かせるから。作るのが好きである
- ~SHOULD~ バックエンドプログラマーとしてやっていきたい。が、必要であれば何でも学ぶ
  - 今までバックエンドをやってきて知見がある。安定して価値を提供できるため
  - フロントエンド: 実際にやってみて、外観へのこだわりや興味があまりないと感じる(やってと言われたらやる)

専門領域。

- ~MUST~ 幅の広さを持ったうえで、そのなかで1つ強い分野を持ったプログラマになりたい(いわゆるT型)

  ここでいう「分野」の **例** 。

  - 「高トラフィック対応に強い」
  - 「レイヤの境界線(OS - ミドルウェア間など)の不具合を解決できる」
  - 「動画配信技術に強い」
  - 「WASMに強い」

  仮定。

  - ググってもわからない、難しい問題に取り組むのは楽しいだろう
  - 同じ志と熱意を持った人と働けるのは楽しいだろう
  - 習熟した概念や道具を組み合わせて作ったり改造するのは楽しいだろう
  - 余暇で楽しみでやっていることと親和性があると楽しいだろう

達成するためのフェーズ。

以上まで書いてきたことは、すぐに得られない。段階を踏んで達成する必要があるように見える。

1. まず多くの仕事や学習をする (👈今ここ)
2. 実際にやってみた経験から、興味や縁を加味して「分野」を徐々にせばめる
3. 1つ強い分野を持つプログラマとして縦横を深めていく
...
- ~MUST~ チームとして開発できる
  - 1人で開発し、永遠にプロジェクトを抜けられないのが辛いと感じた経験から
  - 会社に所属する意味は、同じチームで働き議論し、メンバーから刺激を受けたり知見を交換しあったりすることにあると考えている。なのでチーム開発できなければ会社に所属している意味がない
- ~MUST~ 会社として優れた技術力がある
  - 優れた人や熱意のある人と働きたい
  - 熱意や優秀さは伝播していくと考えている。働くメンバーが自分の成長に影響をもたらすと考えている

業界およびプロダクト。

- ~SHOULD~ コンピュータ資源や開発技術が商材となる業界や会社
  - もっとも興味があり、実際に多くの時間をかけているのがコンピュータである
  - ビジネスに興味を持ちやすく、自分ごととして理解しやすい
- ~SHOULD~ 「整った」受託開発会社
  - 受託開発の経験をして、顧客と新しいものを作っていく体験はよかったと感じた
  - 自社プロダクトの会社と比較して、新しめの技術経験や設計を行いやすいのを好ましく考えている
  - ただし地雷があるのも知っているので、吟味する

* 興味・関心

どちらかというとプライベートの、興味の方向性を示す。
...
3. オープン
  ...
  単にアクセス可能にするだけでなく、時間・言語・文化的背景に依存しない情報をやりとりすることがオープンだと考えている。誰かの役に立つだけでなく、オープンにする人にも大きなメリットがある。よりよくなるための圧力がかかり、さらに多くのフィードバックを受けられる。

1, 2はIT投資家ポール・グレアムのどれかのエッセイで言っていたことで、大学にいる頃からずっとこうやって選ぶようにしてきた。
