...
- 巨大組織の階層型の指揮構造に合わせた、複雑な権限管理が可能な地図アプリケーション
- 要件の変化による、権限構造の改修。今後のビジネスや販売プランに合わせて柔軟に変更できるようにした
- スケールするようにテスト全体の改修。グローバルなfixtureに依存し変更しにくいテストを、各テストケースでレコードを用意するように変更した
- 些細な機能追加。検索フィールドの追加、サムネイル表示の追加など
- 人為的ミスを防ぐための各種ワークフローの追加。テスト追加忘れの検知、OpenAPIやDBスキーマドキュメントの生成忘れの防止
