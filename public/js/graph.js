export function render(jsonfile) {
    d3.json(jsonfile).then(function(data) {
        ...
        const num_colors = Math.max(...data.nodes.map(d => d.communityLabel)) + 1;
        const angleArr = [...Array(num_colors).keys()].map(x => 2 * Math.PI * x / num_colors);
